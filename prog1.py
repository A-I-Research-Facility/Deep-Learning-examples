...
(x_train, y_train), (x_test, y_test) = mnist.load_data()        # loading training and testing data
...
model.add(tf.keras.layers.Dense(128, activation=tf.nn.relu))        # hidden layer 2, similar to hidden layer 1
...
print(val_loss, val_acc)        # the loss is expected to be slightly higher and accuracy is expected to be slightly lower.
...
predictions = new_model.predict([x_test])       # Always remember that predict() always take list as argument
